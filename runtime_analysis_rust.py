...
========================
Builds and runs the Rust runtime-analysis binary, then produces the same
4-panel figure as runtime_analysis.py.
...
BINARY      = RUST_DIR / "target" / "release" / "ystr_analysis"
...
def plot_panel(ax, xs, results, stds, xlabel, panel_label,
    ...
    for name in TOPOLOGIES:
        ax.errorbar(xs, results[name], yerr=stds[name],
                    marker=MARKERS_STYLE[name], color=COLORS[name],
                    linewidth=1.5, markersize=5, capsize=3, elinewidth=0.8,
                    label=name)
        add_linear_fit(ax, xs, results[name], COLORS[name])
...
def make_figure(data: dict) -> None:
    ...
    plot_panel(axes[0, 1], data["us"], data["res_u"], data["std_u"],
               xlabel="Number of untyped males  $n_{unt}$", panel_label="b",
    ...
    fig.legend(handles, labels, loc="lower center",
               bbox_to_anchor=(0.5, 0.01), ncol=3, fontsize=9, frameon=False)
